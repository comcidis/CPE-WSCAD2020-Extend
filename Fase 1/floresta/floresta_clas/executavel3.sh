...
do
	...
	APP="python3.7 floresta_class_3.py ${EXEMPLO[j]} ${ATRIBUTO[k]}"
        $EXECUCAOPERF $COMANDO2 $APP &>Resultado_${EXEMPLO[j]}${ATRIBUTO[k]}ATT$[i].txt
if [ -d "${EXEMPLO[j]}${ATRIBUTO[k]}numerico" ]
then
	mv Resultado_${EXEMPLO[j]}${ATRIBUTO[k]}ATT$[i].txt ${EXEMPLO[j]}${ATRIBUTO[k]}numerico
else
	mkdir ${EXEMPLO[j]}${ATRIBUTO[k]}numerico
	mv Resultado_${EXEMPLO[j]}${ATRIBUTO[k]}ATT$[i].txt ${EXEMPLO[j]}${ATRIBUTO[k]}numerico
fi
done
done
